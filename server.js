...
app.get('/admin/user', (req, res) => {
    res.sendFile(path.join(__dirname, 'protected', 'admin_user.html'));
});

app.get('/admin/driver', (req, res) => {
    res.sendFile(path.join(__dirname, 'protected', 'admin_driver.html'));
});

// Driver Routes
app.get('/driver/login', (req, res) => {
    res.sendFile(path.join(__dirname, 'protected', 'driver_login.html'));
});

app.get('/driver/register', (req, res) => {
    res.sendFile(path.join(__dirname, 'protected', 'driver_register.html'));
});
...
app.post('/api/driver/login', (req, res) => {
    ...
    db.get("SELECT * FROM drivers WHERE username = ? AND mobile = ?", [username, mobile], (err, row) => {
        ...
        if (row) {
            req.session.driverId = row.id;
            req.session.driverName = row.name;
            res.json({ message: 'Login successful', redirect: '/driver' });
        } else {
...
app.get('/driver', isAuthenticatedDriver, (req, res) => {
    res.sendFile(path.join(__dirname, 'protected', 'driver.html'));
});

app.get('/api/driver/session', (req, res) => {
    if (req.session.driverId) {
        res.json({ driverId: req.session.driverId, name: req.session.driverName });
    } else {
...
app.put('/api/bookings/:id/status', (req, res) => {
    ...
    // Get booking and driver info first for emails
    db.get("SELECT * FROM bookings WHERE id = ?", [req.params.id], (err, booking) => {
        if (err || !booking) return res.status(500).json({ error: 'Booking not found' });

        db.run("UPDATE bookings SET status = ?, driverId = ? WHERE id = ?", [status, driverId, req.params.id], function(err) {
            if (err) return res.status(500).json({ error: err.message });
            
            if (driverId) {
                const driverStatus = (status === 'Dispatched' || status === 'Arrived') ? 'Busy' : 'Available';
                ...
                if (booking.email) {
                    if (status === 'Dispatched') {
                        db.get("SELECT * FROM drivers WHERE id = ?", [driverId], (err, driver) => {
                            if (driver) {
                                sendEmail(booking.email, "Ambulance Dispatched", 
                                    `Hello ${booking.patientName},\n\nAn ambulance has been dispatched for your request (#${booking.id}).\n\nDriver: ${driver.name}\nPhone: ${driver.phone}\nVehicle: ${driver.vehicleNumber}\n\nPlease be ready at the pickup location.`);
                            }
                        });
                    } else if (status === 'Arrived') {
                        sendEmail(booking.email, "Ambulance Arrived", 
                            `Hello ${booking.patientName},\n\nYour ambulance has arrived at the pickup location.\n\nPlease look for vehicle ${booking.vehicleNumber || ''}.`);
                    }
                }
            }
